...
async def page_suivante(bot, query):
    ident, req, key, offset = query.data.split("_")
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer(f"Bonjour {query.from_user.first_name},\nNe cliquez pas sur les résultats d'autres utilisateurs !", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    recherche = BUTTONS.get(key)
    cap = CAP.get(key)
    if not recherche:
        await query.answer(f"Bonjour {query.from_user.first_name},\nEnvoyez une nouvelle demande !", show_alert=True)
        return

    fichiers, n_offset, total = await get_search_results(recherche, offset=offset)
    try:
    ...
    if not fichiers:
        return
    temp.FILES[key] = fichiers
    settings = await get_settings(query.message.chat.id)
    del_msg = f"\n\n<b>⚠️ Ce message sera automatiquement supprimé après <code>{get_readable_time(DELETE_TIME)}</code> pour éviter les problèmes de droits d'auteur.</b>" if settings["auto_delete"] else ''
    fichiers_liens = ''

    if settings['links']:
        btn = []
        for num_fichier, fichier in enumerate(fichiers, start=offset + 1):
            fichiers_liens += f"""<b>\n\n{num_fichier}. <a href=https://t.me/{temp.U_NAME}?start=file_{query.message.chat.id}_{fichier.file_id}>[{get_size(fichier.file_size)}] {fichier.file_name}</a></b>"""
    else:
        btn = [[
            InlineKeyboardButton(text=f"📂 {get_size(fichier.file_size)} {fichier.file_name}", callback_data=f'file#{fichier.file_id}')
        ]
            for fichier in fichiers
        ]
    if settings['shortlink'] and not await db.has_premium_access(query.from_user.id):
        btn.insert(0,
            [InlineKeyboardButton("📰 Langues", callback_data=f"languages#{key}#{req}#{offset}"),
            InlineKeyboardButton("🔍 Qualité", callback_data=f"quality#{key}#{req}#{offset}")]
        )
        btn.insert(1,
            [InlineKeyboardButton("♻️ Envoyer tout ♻️", url=await get_shortlink(settings['url'], settings['api'], f'https://t.me/{temp.U_NAME}?start=all_{query.message.chat.id}_{key}')),
            InlineKeyboardButton("🥇 Acheter 🥇", url=f"https://t.me/{temp.U_NAME}?start=plans")]
        )
    else:
        btn.insert(0,
            [InlineKeyboardButton("📰 Langues", callback_data=f"languages#{key}#{req}#{offset}"),
        ...
        btn.insert(1,
            [InlineKeyboardButton("♻️ Envoyer tout", callback_data=f"send_all#{key}#{req}"),
            InlineKeyboardButton("🥇 Acheter 🥇", url=f"https://t.me/{temp.U_NAME}?start=plans")]
        )
    ...
    await query.message.edit_text(cap + fichiers_liens + del_msg, reply_markup=InlineKeyboardMarkup(btn), disable_web_page_preview=True, parse_mode=enums.ParseMode.HTML)

@Client.on_callback_query(filters.regex(r"^languages"))
async def languages_(client: Client, query: CallbackQuery):
    _, key, req, offset = query.data.split("#")
    if int(req) != query.from_user.id:
        return await query.answer(f"Bonjour {query.from_user.first_name},\nNe cliquez pas sur les résultats des autres utilisateurs !", show_alert=True)
    btn = [
    ...
    await query.message.edit_text("<b>Dans quelle langue souhaitez-vous le fichier ? Sélectionnez ici 👇</b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(btn))
...
async def quality(client: Client, query: CallbackQuery):
    ...
    btn.append([InlineKeyboardButton(text="⪻ Retour à la page principale", callback_data=f"next_{req}_{key}_{offset}")])  
    await query.message.edit_text("<b>Quelle qualité souhaitez-vous ? Sélectionnez ici 👇</b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(btn))
...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    cap = CAP.get(key)
    if not search:
        await query.answer(f"Bonjour {query.from_user.first_name},\nEnvoyez une nouvelle requête !", show_alert=True)
        return 

    files, l_offset, total_results = await get_search_results(search, lang=lang)
    if not files:
        await query.answer(f"Désolé, aucun fichier trouvé pour la langue '{lang.title()}' 😕", show_alert=1)
        return
    ...
    if settings['links']:
        btn = []
        for num_fichier, fichier in enumerate(files, start=1):
            fichiers_liens += f"""<b>\n\n{num_fichier}. <a href=https://t.me/{temp.U_NAME}?start=file_{query.message.chat.id}_{fichier.file_id}>[{get_size(fichier.file_size)}] {fichier.file_name}</a></b>"""
    else:
        btn = [[
            InlineKeyboardButton(text=f"📂 {get_size(fichier.file_size)} {fichier.file_name}", callback_data=f'file#{fichier.file_id}')
    ...
    if settings['shortlink'] and not await db.has_premium_access(query.from_user.id):
        ...
        btn.insert(1,
            [InlineKeyboardButton("♻️ Envoyer tout ♻️", callback_data=f"send_all#{key}#{req}"),
            InlineKeyboardButton("🔍 Qualité", callback_data=f"quality#{key}#{req}#{offset}")]
        )
    ...
    if l_offset != "":
        btn.append(
            [InlineKeyboardButton(text=f"1/{math.ceil(int(total_results) / MAX_BTN)}", callback_data="buttons"),
             InlineKeyboardButton(text="Suivant »", callback_data=f"lang_next#{req}#{key}#{lang}#{l_offset}#{offset}")]
        )
    btn.append([InlineKeyboardButton(text="⪻ Retour à la page principale", callback_data=f"next_{req}_{key}_{offset}")])
    await query.message.edit_text(cap + fichiers_liens + del_msg, disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(btn), parse_mode=enums.ParseMode.HTML)

@Client.on_callback_query(filters.regex(r"^lang_next"))
async def lang_next_page(bot, query):
    ident, req, key, lang, l_offset, offset = query.data.split("#")
    if int(req) != query.from_user.id:
        return await query.answer(f"Salut {query.from_user.first_name},\nNe ccliquer pas sur d'autres résultats!", show_alert=True)
    try:
    ...
    cap = CAP.get(key)
    settings = await get_settings(query.message.chat.id)
    del_msg = f"\n\n<b>⚠️ Ce message sera automatiquement Supprimé <code>{get_readable_time(DELETE_TIME)}</code> Pour cause de droites d'auteur</b>" if settings["auto_delete"] else ''
    if not search:
        await query.answer(f"Hello {query.from_user.first_name},\nEnvoyer une autre requête!", show_alert=True)
        return
    ...
    if settings['shortlink'] and not await db.has_premium_access(query.from_user.id):
        ...
        btn.insert(1,
            [InlineKeyboardButton("♻️ Envoyer tout ♻️", url=await get_shortlink(settings['url'], settings['api'], f'https://t.me/{temp.U_NAME}?start=all_{query.message.chat.id}_{key}')),
            InlineKeyboardButton("🔍 Qualité", callback_data=f"quality#{key}#{req}#{l_offset}")]
        )
    else:
        btn.insert(0,[
            InlineKeyboardButton("🥇 Premium 🥇", url=f"https://t.me/{temp.U_NAME}?start=plans")
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("« Retour", callback_data=f"lang_next#{req}#{key}#{lang}#{b_offset}#{offset}"),
             InlineKeyboardButton(f"{math.ceil(int(l_offset) / MAX_BTN) + 1}/{math.ceil(total / MAX_BTN)}", callback_data="buttons")]
        )
    elif b_offset is None:
        btn.append(
            [InlineKeyboardButton(f"{math.ceil(int(l_offset) / MAX_BTN) + 1}/{math.ceil(total / MAX_BTN)}", callback_data="buttons"),
             InlineKeyboardButton("Suivant »", callback_data=f"lang_next#{req}#{key}#{lang}#{n_offset}#{offset}")]
        )
    else:
        btn.append(
            [InlineKeyboardButton("« Retour", callback_data=f"lang_next#{req}#{key}#{lang}#{b_offset}#{offset}"),
...
async def quality_search(client: Client, query: CallbackQuery):
    ...
    if int(req) != query.from_user.id:
        return await query.answer(f"Salut {query.from_user.first_name},\nNe clique pas sur les résultats d'autres personne!", show_alert=True)
    search = BUTTONS.get(key)
    cap = CAP.get(key)
    if not search:
    ...
    if not files:
        await query.answer(f"Désolé '{qual.title()}' Aucun fichier trouvé pour cette langue 😕", show_alert=1)
        return
    temp.FILES[key] = files
    settings = await get_settings(query.message.chat.id)
    del_msg = f"\n\n<b>⚠️ Ce messgae sera automatiquement supprimé <code>{get_readable_time(DELETE_TIME)}</code> Pour cause de droits d'auteur</b>" if settings["auto_delete"] else ''
    files_link = ''
    if settings['links']:
        btn = []
    ...
    if l_offset != "":
        btn.append(
            [InlineKeyboardButton(text=f"1/{math.ceil(int(total_results) / MAX_BTN)}", callback_data="buttons"),
             InlineKeyboardButton(text="Suivant »", callback_data=f"qual_next#{req}#{key}#{qual}#{l_offset}#{offset}")]
        )
    btn.append([InlineKeyboardButton(text="⪻ Retour à la page principale", callback_data=f"next_{req}_{key}_{offset}")])
...
async def quality_next_page(bot, query):
    ...
    if int(req) != query.from_user.id:
        return await query.answer(f"Salut {query.from_user.first_name},\nNe cliquer pas sur les résultats d'autres personne !", show_alert=True)
    try:
        l_offset = int(l_offset)
    except:
        l_offset = 0
...
async def advantage_spoll_choker(bot, query):
    ...
    if files:
        ...
        k = await query.message.edit(f"👋 Hello {query.from_user.mention},\n\nJ'ai pas trouvé <b>'{search}'</b> dans ma base de donnés contact @kingcey. 😔")
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if btn:
            ...
            btn.append(
                [InlineKeyboardButton("🔁 Ressayer 🔁", callback_data=f"checksub#{mc}")]
            )
        ...
        if userid == 0:
            ...
            return
        if userid != user_id:
            await query.answer("Pas pour toi ☠️", show_alert=True)
            return
        btn = await is_subscribed(client, query, settings['fsub'])
        if btn:
        ...
        buttons = [[
            ...
            InlineKeyboardButton('ℹ️ update', url=UPDATES_LINK),
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🧑‍💻 Proprietaire 🧑‍💻', callback_data='owner')
        ],[
